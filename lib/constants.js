...
/*jslint regexp:true*/ // HTML attribute values can be anything
...
(function (root, factory) {
  ...
  return {
    SELECTORS: {
      ...
      EXT: [
        ':animated', ':button', ':checkbox', ':even', ':file', ':first',
        ':header', ':hidden', ':image', ':input', ':last', ':odd', ':parent',
        ':password', ':radio', ':reset', ':selected', ':submit', ':text',
        ':visible'
      ],
      EXT_FN: /:(eq|gt|has|lt)\(/, // extended CSS functions
      EXT_ATTR_NOT_EQUAL: /\[\s*[\w\-]+\s*!=\s*("[^"]*"|'[^']*'|[^\]]*)\]/ // [attr!=value]
    },
    METHODS: {
      CHAIN: [
        'end', 'show', 'hide',
